...
namespace BlackHole.DataProviders
{
    internal class SqliteDataProvider : IDataProvider
    {
        ...
        private int ExecuteEntryScalar<T>(string commandText, T entry)
        {
            try
            {
                int Id = default;
        ...
        public int InsertScalar<T>(string commandStart, string commandEnd, T entry)
        {
            return ExecuteEntryScalar($"{commandStart}){commandEnd}) {insertedOutput};", entry);
        }
